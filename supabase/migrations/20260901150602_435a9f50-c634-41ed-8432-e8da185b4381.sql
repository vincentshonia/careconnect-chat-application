-- Shared, historical definition of "a human was involved" ---------------------
CREATE OR REPLACE FUNCTION public.conversation_human_touched(_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT EXISTS (
      ...
      WHERE c.id = _id AND (
        ...
        OR c.first_human_requested_at IS NOT NULL
        OR c.requested_agent_at IS NOT NULL
        OR c.claimed_at IS NOT NULL
        OR c.assigned_to IS NOT NULL
        OR c.resolved_by IS NOT NULL
        OR c.closed_by IS NOT NULL
        OR c.first_agent_response_at IS NOT NULL
        OR c.last_agent_message_at IS NOT NULL
        OR COALESCE(c.transfer_count, 0) > 0
      ))
    OR EXISTS (
      SELECT 1 FROM public.messages m
      WHERE m.conversation_id = _id AND m.sender_type IN ('agent','staff','human'))
    OR EXISTS (
      SELECT 1 FROM public.conversation_events e
      WHERE e.conversation_id = _id
        AND e.event_type IN ('claimed','assigned','reassigned','transferred','escalated',
                             'human_requested','agent_joined','agent_message'))
$$;

REVOKE ALL ON FUNCTION public.conversation_human_touched(uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.conversation_human_touched(uuid) TO service_role;
...
WITH c AS (
  ...
  -- Spam and archived traffic is not a real service interaction, so it can
  -- neither earn nor cost the assistant a completion.
  SELECT e.*,
  ...
  FROM answered e
  WHERE e.status::text NOT IN ('spam','archived')
)
SELECT jsonb_build_object(
  ...
  'excluded', (SELECT COUNT(*) FROM answered WHERE status::text IN ('spam','archived')),
...
-- Ticket explorer: AI drill-downs + deterministic ordering ----------------------
CREATE OR REPLACE FUNCTION public.report_tickets(_org uuid, _from timestamptz, _to timestamptz,
...
DECLARE
  ...
  ai_answered text := ' AND EXISTS (SELECT 1 FROM public.ai_responses r WHERE r.conversation_id = calc.id)'
    ...
    || ' AND status::text NOT IN (''spam'',''archived'')';
  total bigint;
  rows jsonb;
BEGIN
  sort_col := CASE _sort
